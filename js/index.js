(() => {
  ...
  function findHandler(events, handler3, delegationSelector = null) {
    const uidEventList = Object.keys(events);
    for (let i = 0, len = uidEventList.length; i < len; i++) {
      const event = events[uidEventList[i]];
      if (event.originalHandler === handler3 && event.delegationSelector === delegationSelector) {
        return event;
      }
    }
    return null;
  }
  function normalizeParams(originalTypeEvent, handler3, delegationFn) {
    const delegation = typeof handler3 === "string";
    const originalHandler = delegation ? delegationFn : handler3;
  ...
  function addHandler(element, originalTypeEvent, handler3, delegationFn, oneOff) {
    if (typeof originalTypeEvent !== "string" || !element) {
      return;
    }
    if (!handler3) {
      handler3 = delegationFn;
      delegationFn = null;
    }
    if (customEventsRegex.test(originalTypeEvent)) {
      ...
      if (delegationFn) {
        ...
        handler3 = wrapFn(handler3);
      }
    }
    const [delegation, originalHandler, typeEvent] = normalizeParams(originalTypeEvent, handler3, delegationFn);
    const events = getEvent(element);
    const handlers = events[typeEvent] || (events[typeEvent] = {});
    const previousFn = findHandler(handlers, originalHandler, delegation ? handler3 : null);
    if (previousFn) {
      previousFn.oneOff = previousFn.oneOff && oneOff;
      return;
    }
    const uid = getUidEvent(originalHandler, originalTypeEvent.replace(namespaceRegex, ""));
    const fn2 = delegation ? bootstrapDelegationHandler(element, handler3, delegationFn) : bootstrapHandler(element, handler3);
    fn2.delegationSelector = delegation ? handler3 : null;
  ...
  var EventHandler = {
    on(element, event, handler3, delegationFn) {
      addHandler(element, event, handler3, delegationFn, false);
    },
    one(element, event, handler3, delegationFn) {
      addHandler(element, event, handler3, delegationFn, true);
    },
    off(element, originalTypeEvent, handler3, delegationFn) {
      if (typeof originalTypeEvent !== "string" || !element) {
        return;
      }
      const [delegation, originalHandler, typeEvent] = normalizeParams(originalTypeEvent, handler3, delegationFn);
      ...
      if (typeof originalHandler !== "undefined") {
        ...
        removeHandler(element, events, typeEvent, originalHandler, delegation ? handler3 : null);
        return;
      }
      if (isNamespace) {
  ...
  var __toModule = (module) => {
    return __exportStar(__markAsModule2(__defProp2(module != null ? __create(__getProtoOf(module)) : {}, "default", module && module.__esModule && "default" in module ? { get: () => module.default, enumerable: true } : { value: module, enumerable: true })), module);
  };
  var require_shared_cjs = __commonJS((exports) => {
    ...
    var range = 2;
    function generateCodeFrame(source, start22 = 0, end2 = source.length) {
        ...
        if (count >= start22) {
          for (let j = i - range; j <= i + range || end2 > count; j++) {
            ...
            if (j === i) {
              ...
              const length = Math.max(1, end2 > count ? lineLength - pad : end2 - start22);
  ...
  var require_reactivity_cjs = __commonJS((exports) => {
    ...
    function effect32(fn2, options = shared.EMPTY_OBJ) {
      if (isEffect(fn2)) {
        fn2 = fn2.raw;
      }
      const effect42 = createReactiveEffect(fn2, options);
      if (!options.lazy) {
        effect42();
      }
      return effect42;
    }
    function stop2(effect42) {
      if (effect42.active) {
        cleanup(effect42);
        if (effect42.options.onStop) {
          effect42.options.onStop();
        }
        effect42.active = false;
      }
    }
    var uid = 0;
    function createReactiveEffect(fn2, options) {
      const effect42 = function reactiveEffect() {
        if (!effect42.active) {
          return fn2();
        }
        if (!effectStack.includes(effect42)) {
          cleanup(effect42);
          try {
            enableTracking();
            effectStack.push(effect42);
            activeEffect = effect42;
            return fn2();
          } finally {
            effectStack.pop();
            resetTracking();
      ...
      effect42.id = uid++;
      effect42.allowRecurse = !!options.allowRecurse;
      effect42._isEffect = true;
      effect42.active = true;
      effect42.raw = fn2;
      effect42.deps = [];
      effect42.options = options;
      return effect42;
    }
    function cleanup(effect42) {
      const { deps } = effect42;
      if (deps.length) {
        for (let i = 0; i < deps.length; i++) {
          deps[i].delete(effect42);
        }
        deps.length = 0;
      }
    }
    ...
    function trigger(target, type, key, newValue, oldValue, oldTarget) {
      ...
      const add2 = (effectsToAdd) => {
        if (effectsToAdd) {
          effectsToAdd.forEach((effect42) => {
            if (effect42 !== activeEffect || effect42.allowRecurse) {
              effects.add(effect42);
            }
          });
        }
      };
      ...
      const run = (effect42) => {
        if (effect42.options.onTrigger) {
          effect42.options.onTrigger({
            effect: effect42,
            target,
            key,
            type,
        ...
        if (effect42.options.scheduler) {
          effect42.options.scheduler(effect42);
        } else {
          effect42();
        }
      };
  ...
  function generateEvaluatorFromFunction(dataStack, func) {
    return (receiver = () => {
    }, { scope = {}, params: params2 = [] } = {}) => {
      let result = func.apply(mergeProxies([scope, ...dataStack]), params2);
  ...
  function generateEvaluatorFromString(dataStack, expression) {
    ...
    return (receiver = () => {
      ...
      if (func.finished) {
        runIfTypeOfFunction(receiver, func.result, completeScope, params2);
      } else {
        promise.then((result) => {
          runIfTypeOfFunction(receiver, result, completeScope, params2);
        });
      }
    };
  }
  function runIfTypeOfFunction(receiver, value, scope, params2) {
    if (typeof value === "function") {
      let result = value.apply(scope, params2);
      if (result instanceof Promise) {
        result.then((i) => runIfTypeOfFunction(receiver, i, scope, params2));
      } else {
        receiver(result);
      }
    } else {
...
`, el);
  ...
  function transition(el, setFunction, { during, start: start22, end: end2, entering } = {}, before = () => {
  }, after = () => {
  }) {
  ...
  function on(el, event, modifiers, callback) {
    ...
    if (modifiers.includes("debounce")) {
      ...
      handler3 = debounce2(handler3, wait, this);
    }
  ...
  directive("for", (el, { expression }, { effect: effect32, cleanup }) => {
  ...
  directive("if", (el, { expression }, { effect: effect32, cleanup }) => {
    let evaluate2 = evaluateLater(el, expression);
    let show = () => {
